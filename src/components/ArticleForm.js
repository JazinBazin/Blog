...
class ArticleForm extends React.Component {
    ...
    handleHeadlineChange = e => { this.setState({ headline: e.target.value }); }
    ...
    handleSubmitClicked = () => {
        const headline = this.articleHeadline.current.value;
        if (!headline || headline.length >= 100) {
            alert("Headline length >= 100");
            return;
        }
        const text = this.articleText.current.value;
        if (!text || text.length >= 1000) {
            alert("Text length >= 500");
            return;
        }
        const creationDate = Date.now();
    ...
    handleResetClicked = () => {
        this.articleHeadline.current.value = '';
                ...
                <form>
                    <div className="form-group">
                        ...
                        <input
                            ...
                            type="text"
                            className="form-control"
                            defaultValue={this.state.article.headline}
                        />
                    </div>
                    <div className="form-group">
                        <label>Текст статьи:</label>
